...
def quick_actions_keyboard(role: UserRole,
    ...
    rows: list[list[InlineKeyboardButton]] = [
        ...
        [InlineKeyboardButton(text="Добавить занятие",
                              callback_data="ux:add_lesson")],
    ]

    if not is_group:
        rows.append([InlineKeyboardButton(text="Добавить напоминание",
                                          callback_data="ux:add_reminder")])
        rows.append([InlineKeyboardButton(text="Удалить напоминание",
                                          callback_data="ux:remove_reminder")])
    ...
    if role == UserRole.OWNER:
        rows.append(
            [InlineKeyboardButton(text="SQL консоль", callback_data="ux:sql")])
