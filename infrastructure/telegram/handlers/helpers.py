...
from domain.exceptions import UserNotRegisteredException
...
async def get_or_create_user(event: Message | CallbackQuery) -> tuple[
    User, bool]:
    """
    Получает или создаёт пользователя.
    1. Ищет по telegram_id.
    2. Если нет, ищет по username (для тех, кого добавили в группе с telegram_id=0) -> обновляет telegram_id.
    3. Если нет и это Owner/Admin -> создает.
    4. Иначе -> кидает UserNotRegisteredError.
    """
    ...
    # Получаем роль из конфига/настроек (как было у вас)
    ...
    user_by_name = await user_repo.get_by_username(username)
    if user_by_name:
        # Нашли пользователя, у которого telegram_id = 0. Обновляем на реальный.
        user_by_name.telegram_id = user_id
        logger.info(f"IIIIIIIIIIIIIIIIIIIIIIIIIIIID:{user_id}")
        await user_repo.update(user_by_name)
        return user_by_name, False
