...
from domain.entities import LessonStatus
...
@router.message(_GROUP, Command("done"),
                StateFilter(GroupRegSG.waiting_for_username))
async def group_reg_done_early(message: Message, state: FSMContext):
    data = await state.get_data()
    # Проверяем, что пишет тот же человек, кто начал (/start)
    if message.from_user.id != data.get("initiator_id"):
        return

    await state.clear()

    # Получаем пользователя, чтобы определить его роль для меню
    user, _ = await get_or_create_user(message)

    await message.answer(
        "✅ Регистрация участников завершена!\n\n"
        "Теперь вы можете назначать занятия и управлять чатом:",
        reply_markup=main_menu_keyboard(user.role, is_group=True)
    )
    await message.answer(
        "🚀 Быстрые действия:",
        reply_markup=quick_actions_keyboard(user.role, is_group=True)
    )
...
@router.message(
    _GROUP,
    Command("done"),
    StateFilter(GroupRegSG.role_selection, GroupRegSG.name_input,
...
async def group_reg_done_anytime(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Меню для управления занятиями доступно ниже:",
        reply_markup=main_menu_keyboard(user.role, is_group=True)
    )
    await message.answer(
        "🚀 Быстрые действия:",
...
async def group_reg_confirmation(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "no":
        await state.set_state(GroupRegSG.role_selection)
        await callback.message.edit_text(f"Кто @{username}?",
    ...
    if existing:
        ...
        existing.role = role
        # Если в БД стоит заглушка 0, а мы узнали реальный ID — обновляем
    ...
    if not member_exists:
        ...
        await chat_member_repo.create(ChatMember(
            chat_id=chat_id,
            user_id=linked_user.telegram_id,  # FK теперь хранит username
            is_active=True,
        ))
...
async def on_start_lesson(callback: CallbackQuery):
    """Обработчик кнопки 'Начать занятие'"""
    lesson_id = int(callback.data.split(":")[1])

    # 1. Проверяем права: только учитель/админ может начать
    user, _ = await get_or_create_user(callback)
    if user.role not in (UserRole.TEACHER,UserRole.ADMIN , UserRole.OWNER):
        await callback.answer("❌ Только преподаватель может начать занятие",
                              show_alert=True)
        return
    ...
    # 3. Обновляем: ставим статус IN_PROGRESS и фиксируем время начала
    ...
    new_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Завершить",
                              callback_data=f"complete_lesson:{lesson_id}")]
    ])
...
async def on_complete_lesson(callback: CallbackQuery):
    """Обработчик кнопки 'Завершить занятие'"""
    lesson_id = int(callback.data.split(":")[1])

    # 1. Проверяем права
    user, _ = await get_or_create_user(callback)
    if user.role not in (UserRole.TEACHER, UserRole.ADMIN, UserRole.OWNER):
        await callback.answer("❌ Только преподаватель может завершить занятие",
                              show_alert=True)
        return
    ...
    # 3. Обновляем: ставим статус COMPLETED и фиксируем время конца
    lesson.status = LessonStatus.COMPLETED
    lesson.actual_end = datetime.now(timezone.utc)

    # Считаем длительность, если есть actual_start
    ...
    # 4. Редактируем сообщение: финальный вид
    await callback.message.edit_text(
